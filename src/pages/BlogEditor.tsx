...
const BlogEditor = () => {
  ...
  const fetchArticles = async () => {
    setLoading(true);
    try {
      const saved = localStorage.getItem("admin_blog_articles");
      if (saved) {
        ...
        const defaultArticles: Article[] = [
          {
            id: "1",
            title: "Exploring Singapore Consolidated Sea Freight Services",
            excerpt: "Learn how consolidated sea freight can save business costs in Southeast Asia shipping lines.",
            content: "Full content about Singapore Consolidated Sea Freight Services.",
            slug: "exploring-singapore-consolidated-sea-freight",
            featured_image: "https://images.unsplash.com/photo-1578575437130-527eed3abbec?auto=format&fit=crop&q=80&w=600",
            published_at: new Date().toISOString(),
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
            meta_title: "Singapore Consolidated Sea Freight Services",
            meta_description: "Learn how consolidated sea freight can save business costs in Southeast Asia shipping lines.",
            alt_text: "Singapore Consolidated Sea Freight",
            tags: ["sea freight", "singapore", "consolidation"]
          }
        ];
        setArticles(defaultArticles);
        localStorage.setItem("admin_blog_articles", JSON.stringify(defaultArticles));
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      let featuredImage = imagePreview;
      if (selectedFile) {
      ...
      if (editingId) {
        updatedArticles = articles.map(a => 
          a.id === editingId ? { ...a, ...articleData, updated_at: new Date().toISOString() } : a
        );
      ...
      localStorage.setItem("admin_blog_articles", JSON.stringify(updatedArticles));
  ...
  const fetchGalleryImages = async () => {
    ...
    try {
      ...
      if (saved) {
        ...
        setGalleryImages(allImages.filter(img => img.country === selectedCountry));
      } else {
        ...
        setGalleryImages(defaultImages.filter(img => img.country === selectedCountry));
      }
    } catch (error: any) {
  ...
  const handleGalleryFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentList: GalleryImage[] = allSaved ? JSON.parse(allSaved) : [];
      ...
      if (galleryUploadForm.country === selectedCountry) {
        setGalleryImages(newList.filter(img => img.country === selectedCountry));
      }
    } catch (error: any) {
  ...
  const handleUpdateGalleryImage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setGalleryImages(updated.filter(img => img.country === selectedCountry));
    } catch (error: any) {
            ...
            <div>
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {tags.map((tag, index) => (
                    <span key={index} className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm bg-red-100 text-red-800">
                      #{tag}
                      <button type="button" onClick={() => handleRemoveTag(tag)} className="ml-1 text-red-600 hover:text-red-800">
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ))}
  ...
  if (isInsideAdmin) {
    return (
      <div className="bg-white">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Blog & Gallery Editor</h1>
