...
const ContactSidebarB: React.FC<ContactSidebarProps> = ({ isOpen, onClose }) => {
  ...
  const [mapUrl, setMapUrl] = useState("https://www.google.com/maps/d/u/0/embed?mid=1Gy9JUvlSaOBrtQaKI7OoYU2KgFymoXg&ehbc");
  ...
  useEffect(() => {
    ...
    if (savedCountries) {
      try {
        ...
        if (Array.isArray(parsed)) setCountriesList(parsed);
      } catch (e) {
        console.error(e);
      }
    }
    const savedMap = localStorage.getItem("admin_global_presence_map_url_bangladesh") || localStorage.getItem("admin_global_presence_map_url");
  ...
  useEffect(() => {
    if (sortedCountries.length > 0 && sortedCountries[0].cities && sortedCountries[0].cities.length > 0) {
